...
const uint32_t N_B = FFT_L / 2 / AUDIO_BLOCK_SAMPLES;
...
void setup() {
  Serial.begin(115200);
  while(!Serial && millis() < 2000);   // must patch out for Visual Micro
...
void setupSGTL5000() {
	...
	codec.volume(0.6); //   does affect headphone volume
...
void show_memory_and_load(void)
{
    Serial.print("Proc = ");
    Serial.print(AudioProcessorUsage() * SAMPLE_RATE / AUDIO_SAMPLE_RATE_EXACT);
    Serial.print("% (");
    Serial.print(AudioProcessorUsageMax() * SAMPLE_RATE / AUDIO_SAMPLE_RATE_EXACT);
    Serial.print("%),  Mem = ");
